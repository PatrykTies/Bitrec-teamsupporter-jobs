  ...
  displayAllFormValues(){
    if(this.props.all_values){
      ...
      return _.map(all_values_copy, function(value, key) {
        ...
        return <div key={pairToReturn.key}><span>{pairToReturn.key}</span>{": "}<span>{pairToReturn.value}</span></div>
      });
    }
  }
      ...
      <div>
        <Row center="xs" style={{height: 460, width: '80%', margin: '0 auto'}}>
          ...
          <div style={{height: 'calc(100% - 300px)', overflowY: 'scroll',
            ...
            {this.props.answers_to_extra_questions && this.props.answers_to_extra_questions !== [] && this.props.answers_to_extra_questions.forEach(question=>{

                if(this.props.additionalQuestionsState){

                  return this.props.additionalQuestionsState.forEach(questionFromAll=>{
